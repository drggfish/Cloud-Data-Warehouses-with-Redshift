import configparser
import psycopg2
from sql_queries import drop_table_queries, copy_table_queries, \
                        insert_table_queries, all_tables_count_rows
...
def count_rows(cur, conn):
    """
    Test to determine if all tables were populated with data successfully
    """
...
def main():
    ...
    conn = psycopg2.connect("host={} dbname={} user={} password={} \
                            port={}".format(*config['DB'].values()))
    cur = conn.cursor()

    print("Issuing copy commands (it may take up to 15 minutes time to load \
                                  the data from s3). Please wait ...")
